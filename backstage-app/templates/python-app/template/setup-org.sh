...
# ---------------------------------------------------------------------------
# Helpers — org-level existence checks
# ---------------------------------------------------------------------------
org_secret_exists() {
  gh secret list --org "$ORG" --json name --jq '.[].name' 2>/dev/null | grep -qx "$1"
}
org_variable_exists() {
  gh variable list --org "$ORG" --json name --jq '.[].name' 2>/dev/null | grep -qx "$1"
}
...
# ---------------------------------------------------------------------------
# Step 2 — Set GitHub Actions secrets (org-level, skip if already present)
# ---------------------------------------------------------------------------
echo ""
...
for name in DOCKERHUB_USERNAME DOCKERHUB_TOKEN ARGOCD_PASSWORD GH_PAT; do
  if org_secret_exists "$name"; then
    echo "  $name already exists in org $ORG — skipping."
  else
...
# ---------------------------------------------------------------------------
# Step 3 — Set GitHub Actions variables (org-level, skip if already present)
# ---------------------------------------------------------------------------
echo ""
...
for name in ARGOCD_VERSION YQ_VERSION KUBECTL_VERSION; do
  if org_variable_exists "$name"; then
    echo "  $name already exists in org $ORG — skipping."
  else
