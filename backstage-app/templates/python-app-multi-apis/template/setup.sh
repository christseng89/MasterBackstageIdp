...
#
# Race-condition note: Windows briefly locks the hosts file after a write
# (DNS Client service, Defender real-time scan) — back-to-back `>>` from a
# per-line loop hits "Device or resource busy" on the 2nd/3rd line.  We
# collect missing entries first, then write them all in ONE append, with
# retries, to dodge the lock.
echo ""
...
for name in "${HOST_NAMES[@]}"; do
  if grep -q "$name" "$HOSTS"; then
    ...
    ENTRIES_TO_ADD+=("127.0.0.1 $name")
    NEED_WRITE=true
  fi
done

# Note: avoid bash array-length syntax (dollar-brace-hash) inside files
# processed by Backstage scaffolder — Nunjucks treats brace-hash as a
# comment opener and fails with "expected end of comment".
# A simple boolean flag dodges the conflict cleanly.
if [ "$NEED_WRITE" = true ]; then
  success=false
  for attempt in 1 2 3 4 5; do
    if printf '%s\n' "${ENTRIES_TO_ADD[@]}" >> "$HOSTS" 2>/dev/null; then
      success=true
      break
    fi
    echo "⚠  hosts file busy (Windows DNS / Defender lock), retry $attempt/5 in 1s..."
    sleep 1
  done
  ...
  if [ "$success" = true ]; then
    ...
    echo "✗ Failed to write hosts after 5 retries. Add manually from an"
    echo "  Administrator PowerShell:"
    for entry in "${ENTRIES_TO_ADD[@]}"; do
      echo "    Add-Content -Path 'C:\\Windows\\System32\\drivers\\etc\\hosts' -Value '$entry'"
    done
    exit 1
  fi
fi
