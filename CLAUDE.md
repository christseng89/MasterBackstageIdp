...
| `backstage-app/templates/python-app/README-gitWorkflows.md` | CI/CD workflows generated by the scaffolder template |
...
- `.github/workflows/<app_name>-cicd.yaml` — CI + auto-deploy to **dev**
- `.github/workflows/<app_name>-cd.yaml` — manual-promotion deploy to **staging/prod**
...
| `GH_PAT` | `python-app` CD job (git push back to main); scaffolded repos use `GITHUB_TOKEN` instead |

**Repository Variables** (Settings → Variables, distinct from secrets):

| Variable | Purpose |
|----------|---------|
| `ARGOCD_VERSION` | ArgoCD CLI version pulled from Docker Hub mirror (e.g. `v3.4.2`) |
| `YQ_VERSION` | yq version (e.g. `v4.44.3`) |
| `KUBECTL_VERSION` | kubectl version (scaffolded repos only) |
