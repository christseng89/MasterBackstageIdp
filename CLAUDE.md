...
| `README-Recap2Psql.md` | PostgreSQL setup for Backstage: role creation, `.env` vars, `app-config.local.yaml` override, Docker run command |
...
`backstage-app/templates/python-app-multi-apis/template.yaml` provisions a Flask microservice that demonstrates the **additive multi-version API pattern**: one image simultaneously serves six API versions with distinct lifecycle states, routed entirely by Helm env vars — no per-version Docker builds. Skeleton at `backstage-app/templates/python-app-multi-apis/template/`.

Key differences from python-app:
- **Six Backstage API entities** (type: openapi) registered alongside the Component — each with its own `lifecycle` value and inline OpenAPI spec in `catalog-info.yaml`
- **Flask blueprints per version** (`src/api_v1.py` … `api_v6.py`); `app.py` reads four Helm-injected env vars at startup to decide which blueprints to register:
  - `ENABLED_VERSIONS` — comma list of versions served normally (e.g. `"v3,v4,v5,v6"`)
  - `DEPRECATED_VERSIONS` — subset of `ENABLED_VERSIONS`; also receives RFC 9745 headers (e.g. `"v2"`)
  - `REMOVED_VERSIONS` — versions registered as 410 Gone catch-all (e.g. `"v1,v2"`)
  - `DEFAULT_API_VERSION` — surfaced on K8s labels + `/version` metadata (e.g. `"v4"`)
- **RFC 9745/8594 deprecation headers** injected by `src/deprecated_headers.py` after-request middleware; reads `DEPRECATED_VERSIONS` from env (injected by Helm) and matches against `SUNSET_INFO` in `removed_handlers.py` for metadata
- **`GET /version` endpoint** (`src/version_endpoint.py`) returns image tag, git SHA, enabled/deprecated/removed version lists, and Backstage entity refs; reads `IMAGE_TAG`, `GIT_SHA`, `APP_NAME`, `ENABLED_VERSIONS`, `DEPRECATED_VERSIONS`, `REMOVED_VERSIONS`, `DEFAULT_API_VERSION` from env
- **`src/removed_handlers.py`** — `make_removed_blueprint(version)` creates a Flask blueprint that intercepts all methods on `/api/<version>/*` and returns `HTTP 410 Gone` with `Sunset` + `Link: rel="successor-version"` headers and a migration guide URL. Use 410 not 404: it signals the endpoint existed and was intentionally retired

API route summary:

| Version | Lifecycle | Net-new endpoints |
|---------|-----------|-------------------|
| v1 | retired (410 Gone) | `/info`, `/healthz`, `/hostname` |
| v2 | deprecated (sunset 2026-06-30) | `/echo`, `/time` |
| v3 | production | `/random`, `/uptime`, `/env` |
| v4 | production (default) | `/metrics` |
| v5 | production | `/list` (paginated) |
| v6 | experimental | `/events` (SSE stream) |

Per-env routing matrix (controlled by `values-{dev,staging,prod}.yaml`):
...
To deprecate a version: add it to `DEPRECATED_VERSIONS` in the relevant `values-{env}.yaml` and flip `lifecycle` in `catalog-info.yaml`. To retire a version: move it from `ENABLED_VERSIONS` to `REMOVED_VERSIONS` in `values-{env}.yaml` — the `removed_handlers.py` catch-all takes over automatically. Update `SUNSET_INFO` in `removed_handlers.py` with the sunset date before retiring.
